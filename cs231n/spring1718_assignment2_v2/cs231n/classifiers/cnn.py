...
class ThreeLayerConvNet(object):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
        ...
        # 卷积得到的图层和输入一样，池化后剩下一半。                                   #
        ############################################################################
        C,H,W = input_dim
        self.params['W1'] = np.random.normal(loc=0, scale=weight_scale, size=(num_filters, C, filter_size,filter_size))
        self.params['b1'] = np.zeros(num_filters)
        self.params['W2'] = np.random.normal(loc=0, scale=weight_scale, size=(int((H / 2)*(W / 2)*num_filters), hidden_dim))
    ...
    def loss(self, X, y=None):
        ...
        h_out, h_cache = affine_relu_forward(pool_out, W2, b2)
        scores, cache = affine_forward(h_out, W3, b3)
        pass
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        dh, dw3, db3 = affine_backward(dscores, cache)
        dpool, dw2, db2 = affine_relu_backward(dh, h_cache)
